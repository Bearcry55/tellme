...
export default function CatTemplate({ data }: TemplateProps) {
  ...
  // Embedded lightweight animated fallback links
  ...
  const getNoButtonText = () => {
    const phrases = [
      ...
      "Pookie please... think again",
      "Don't do this to me 😭",
      "I will literally cry",
      "Look at the cats, how can you say no?",
      "Error: Wrong button clicked 😉"
    ];
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-rose-100 p-4 text-black text-center">
        <img 
          ...
          className="w-56 h-56 mb-4 object-contain" 
        />
        <h1 className="text-4xl font-extrabold text-rose-600 animate-pulse">
          YAAAAAY!!! 🎉
        </h1>
        <p className="text-xl mt-2 text-gray-700">
          Take a screenshot and send it to <span className="font-bold text-rose-500">{data.sender_name}</span> right away!
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-rose-50 p-4 text-black text-center">
      ...
      <p className="text-lg text-gray-600 mb-8 max-w-sm">
        {data.sender_name} has a super critical, life-altering question for you...
      </p>

      <h2 className="text-2xl font-extrabold text-rose-600 mb-8">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center gap-4 w-full max-w-xs mx-auto px-4">
        ...
        <button
          className="bg-red-500 hover:bg-red-600 text-white font-semibold py-2 px-4 rounded-xl shadow-md transition whitespace-nowrap"
